package com.test.test1.lbtransaction.connection;
 ...
 * @description:   把自定义的Connection给Spring用  拿到事务的控制权
 * @author: 翟飞
 * @create: 2019-07-31 00:33
 **/
public class LbConnection implements Connection {

    private Connection connection;    // Spring的代理连接
    private LbTransaction lbTransaction;  // 事务对象
    ...
    public void commit() throws SQLException {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    //  如果通知是commit的话就提交
                    if(TransactionType.commit.equals(lbTransaction.getTransactionType())){
                        // commit
                        connection.commit();
                    }else {
                        connection.rollback();
    ...
    public void rollback() throws SQLException {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    //  如果通知是commit的话就提交
                    if(TransactionType.rollback.equals(lbTransaction.getTransactionType())){
                        connection.rollback();
                    }
                    connection.close();
                } catch (SQLException e) {
